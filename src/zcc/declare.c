...
PRIVATE void idecllist()
{
					...
					gvarsymptr->level == GLBLEVEL))
	{
		...
		gvarsymptr->flags &= ~EXTERNAL;
		 ...
		 gvarsc == EXTERNDECL)
	{
		    gvarsymptr = _zcc_addglb(gvarname, gvartype);
	...
	else
	{
		...
		gvarsc != TYPEDEFDECL)
#ifdef KCPSM
		if( autoVarPos+gvartype->typesize > globalVarPos )
			printf(";; too many auto values auto value pos = %d global value pos = %d new auto value size is  %d\n",autoVarPos,globalVarPos,gvartype->typesize);	
		gvarsymptr->offset.offi = autoVarPos;
		autoVarPos = autoVarPos+gvartype->typesize;
#else
