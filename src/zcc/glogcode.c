...
# define posindependent 0
	...
	if( *pcondtrue==LS || *pcondtrue==HI )
	{
		...
		*pcondtrue = reverscc[*pcondtrue];
    	...
    	if( *pcondtrue==NE || *pcondtrue==EQ )
    		_zcc_cmporsub(source,target,1);
    	else
    		_zcc_cmporsub(source,target,0);
